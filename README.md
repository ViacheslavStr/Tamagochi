...
Симуляция «ребёнка» для пары: загрузка фото/видео и анкеты родителей → ИИ формирует модель ребёнка (внешность + характер) → ребёнок растёт, общается и выражает эмоции в приложении. Можно также загрузить свои фото и фото знаменитости или друга — чтобы «увидеть», как бы выглядел общий ребёнок.
...
- **Состояние:** БД PostgreSQL — возраст, личность, память, события; симуляция времени.
