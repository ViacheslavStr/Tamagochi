...
Пара загружает фото/видео и заполняет **по одной анкете на каждого** (внешность/характер родителя 1 и родителя 2) → ИИ формирует модель «ребёнка» (внешность + базовые черты из обеих анкет) → ребёнок «живёт» в приложении: растёт, общается, выражает эмоции в зависимости от возраста и воспитания.

**Расширенный сценарий:** пользователь может загрузить свои фото и фото знаменитости или друга — чтобы «увидеть», как бы выглядел их общий ребёнок. Один «родитель» — сам пользователь, второй — выбранное лицо (знаменитость, друг, партнёр). Те же механизмы генерации лица и **две анкеты** (по одной на каждого) применяются; результат — симуляция ребёнка от двух заданных «родителей» (в т.ч. для развлечения и экспериментов, без привязки к реальному партнёру).
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                         РОДИТЕЛИ / ПАРА (онбординг)                      │
├─────────────────────────────────────────────────────────────────────────┤
│  Фото/видео родителя 1  │  Фото/видео родителя 2  │  Анкета 1  │  Анкета 2  │
│  (вы или партнёр)       │  (партнёр, друг,        │  (гены/    │  (гены/    │
│                         │   знаменитость)         │   характер)│   характер)│
└────────┬────────────────┴──────────┬──────────────┬───────────┘
         │                    │              │             │
         ▼                    ▼              ▼             ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
│  • Вывод базовых характеристик из анкет обоих родителей                  │
...
- **Сущности:**
  ...
  - **Traits:** производные от анкет обоих родителей + возможные мутации/вариации.
...
- **Характер:** базовый вектор из анкет обоих родителей (усреднение/комбинация); опционально — лёгкая корректировка от типа взаимодействия (больше похвалы → чуть выше «добросовестность» в следующем сеансе и т.д.).
...
1. **Этап 1 — Онбординг и генерация:** анкеты обоих родителей (по одной на каждого), загрузка фото, вызов API генерации лица ребёнка (хотя бы один возраст), сохранение профиля и сгенерированных картинок.
...
- **Этика:** продукт позиционировать как симуляция/развлечение, не как предсказание реального ребёнка; в анкетах и интерфейсе явно указывать это.
...
## 7. Технические долги и решения на будущее

### 7.1 Хранение медиафайлов (фото/видео)

**Текущее решение:** Локальное хранение в `apps/backend/uploads/` с раздачей через статику NestJS.

**TODO:** Обдумать и принять решение по стратегии хранения:
- **Вариант A:** Оставить локальное хранение (простота, контроль, бесплатно)
  - Плюсы: нет зависимости от внешних сервисов, полный контроль
  - Минусы: нужны резервные копии, сложнее масштабирование, нет CDN
- **Вариант B:** Перейти на внешний сервис (Cloudinary, AWS S3, Cloudflare R2)
  - Плюсы: CDN, автоматические бэкапы, лучшее масштабирование
  - Минусы: зависимость от сервиса, возможные затраты при росте

**Решение:** Оставить локальное хранение для MVP, пересмотреть при росте нагрузки или необходимости CDN.
